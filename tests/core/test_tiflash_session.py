...
class TestTIFlashSession:
    ...
    class TestCore:
        ...
        def test_core_read_data(self, tenv, tdev):
            """Tests simple core.read_data()"""
            core_name = tdev['session']
            ccxml = tdev['ccxml-path']
            device = tiflash.TIFlashSession(ccxml=ccxml)
            ...
            core = device.get_core(core_name)
            core.connect()
            result = core.read_data(tdev['read-address'])
            assert len(result) == 1

        def test_core_read_data_multiple_bytes(self, tenv, tdev):
            """Tests simple core.read_data(num_bytes=4)"""
            core_name = tdev['session']
            ccxml = tdev['ccxml-path']
            device = tiflash.TIFlashSession(ccxml=ccxml)
            ...
            core = device.get_core(core_name)
            core.connect()
            result = core.read_data(tdev['read-address'], num_bytes=4)
            assert len(result) == 4
        ...
        def test_core_write_data(self, tenv, tdev):
            """Tests simple core.write_data()"""
            core_name = tdev['session']
            ccxml = tdev['ccxml-path']
            device = tiflash.TIFlashSession(ccxml=ccxml)
            ...
            core = device.get_core(core_name)
            core.connect()
            core.write_data(data=[0xFF, 0xFF], address=int(tdev['write-address'], 16))
