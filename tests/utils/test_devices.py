...
class TestDevices:
    def test_get_devices_directory(self, tenv):
        expected = os.path.normpath(
            tenv["paths"]["ccs"] + "/ccs_base/common/targetdb/devices"
        )
    ...
    def test_get_devicetype(self, tenv):
        expected = "CC1350F128"
        devicexml = os.path.normpath(
            tenv["paths"]["ccs"] + "/ccs_base/common/targetdb/devices" "/cc1350f128.xml"
        )
    ...
    def test_get_cpu_xml(self, tenv):
        expected = os.path.normpath(
            tenv["paths"]["ccs"] + "/ccs_base/common/targetdb/cpus/" "cortex_m3.xml"
        )
    ...
    def test_get_default_connection_xml(self, tenv):
        ...
        result = devices.get_default_connection_xml(device_xml, tenv["paths"]["ccs"])
