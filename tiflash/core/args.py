...
SessionParser.add_argument(
    "--connection", default=None, help="Connection type to use for device"
)
...
SessionParser.add_argument(
    "-F", "--fresh", action="store_true", help="Generate new (fresh) ccxml"
)
SessionParser.add_argument(
    "-D", "--debug", action="store_true", help="Display debugging output"
)
SessionParser.add_argument(
    "-A",
    "--attach",
...
OptionsGetParser.add_argument(
    "optionID", metavar="optionID", help="Option ID to get value of."
)
OptionsGetParser.add_argument(
    "-op",
    "--operation",
    metavar="preOperation",
    help="""Specify an operation to perform prior to
                            getting option""",
)

OptionsSetParser = argparse.ArgumentParser(add_help=False)
OptionsSetParser.add_argument(
    "optionID", metavar="optionID", help="Option ID to set value of."
)
OptionsSetParser.add_argument(
    "optionVal", metavar="optionVal", help="Value to set option to."
)
OptionsSetParser.add_argument(
    "-op",
    "--operation",
    metavar="postOperation",
    help="""Specify an operation to perform after
                            setting option""",
)

OptionsListParser = argparse.ArgumentParser(add_help=False)
OptionsListParser.add_argument(
    "optionID", metavar="optionID", nargs="?", help="Option ID to get info on."
)

# List Parser
ListParser = argparse.ArgumentParser(add_help=False)
ListParser.add_argument(
    "--devicetypes", action="store_true", help="Prints list of installed devicetypes"
)
ListParser.add_argument(
    "--connections", action="store_true", help="Prints list of installed connections"
)
ListParser.add_argument(
    "--cpus", action="store_true", help="Prints list of installed cpus"
)
# ListParser.add_argument('--cfgs', action='store_true',
#                        help='Prints list of installed target cfgs')
ListParser.add_argument(
    "--options", action="store_true", help="Prints list of target options"
)
ListParser.add_argument(
    "-f",
    "--filter",
...
ResetParser.add_argument(
    "-o",
    "--option",
    nargs=2,
    action="append",
    dest="options",
    metavar="optionID optionValue",
...
VerifyParser.add_argument(
    "image", metavar="image", nargs=1, help="""Image to verify."""
)
VerifyParser.add_argument(
    "-b", "--bin", action="store_true", help="Specify if image is a binary image"
)
...
FlashParser.add_argument(
    "-b", "--bin", action="store_true", help="Specify if image(s) are binary images"
)
FlashParser.add_argument(
    "-a", "--address", metavar="address", help="Address to begin flashing image(s)"
)
FlashParser.add_argument(
    "-o",
    "--option",
...
MemoryReadParser.add_argument(
    "-p", "--page", default=0, help="Page number in memory to access address"
)
MemoryReadParser.add_argument(
    "-n", "--num", dest="num_bytes", default=1, help="Number of bytes to read"
)
MemoryReadParser.add_argument(
    "--hex", action="store_true", help="Displays output in hex"
)
...
MemoryWriteParser.add_argument(
    "-p", "--page", default=0, help="Page number in memory to access address"
)
MemoryWriteParser.add_argument(
    "-d",
...
RegisterReadParser.add_argument(
    "--hex", action="store_true", help="Displays output in hex"
)
...
RegisterWriteParser.add_argument(
    "value", help="Value (32bit hex) to write to register."
)
